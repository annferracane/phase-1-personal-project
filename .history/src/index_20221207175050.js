...
const displayAllJokes = function displayJokes() {
    fetch(localUrlBase)
    .then(resp => resp.json())
...
function refreshJokesHandler() {
    ...
    clearJokeDatabase(fetchJokes, displayJokes);
}

function clearJokeDatabase(fetch, display) {
    fetch(localUrlBase)
    .then(resp => resp.json())
...
const fetchJokes = function fetchNewJokes() {
