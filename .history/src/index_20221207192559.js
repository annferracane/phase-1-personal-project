...
refreshJokesBtn.addEventListener('click', e => fetchNewJokes(e));
...
function deleteJokesHandler() {
    ...
    setTimeout(() => {clearJokeDatabase(); }, 2000);
    setTimeout(() => {fetchNewJokes(); }, 2000);
    setTimeout(() => {displayJokes(); }, 2000);
}
...
function fetchNewJokes(e) {
    ...
    .then(resp => resp.json())
    .then(jokeAPIData => addJokeToDatabase(jokeAPIData.jokes).then)
...
function addJokeToDatabase(jokeAPIData) {
    ...
    jokeAPIData.forEach(jokeAPI => {
        ...
        .catch(error => `Error in addJokeToDatabase function: ${console.log(error)}`);
    });
}
