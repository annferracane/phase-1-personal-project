...
function refreshJokesHandler() {
    let results = await Promise.all([clearJokeDatabase(), fetchNewJokes(), function3()]);
    // Delete jokes from local db and dom, fetch new jokes, display jokes
    //jokeCardsDiv.innerHTML = '';
    //const clearJokes = clearJokeDatabase;
    //.then(resp => console.log('delete finished'));

    //fetchNewJokes();

    // Display new jokes on DOM
    //displayJokes();
}

function async clearJokeDatabase() {
    fetch(localUrlBase)
    .then(resp => resp.json())
